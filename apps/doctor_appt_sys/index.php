...
// Assuming logger.php initializes the $log object and returns it
$log = require_once 'logger.php';

// Check if user is already logged in
if (isset($_SESSION['patientSession']) != "") {
    header("Location: patient/patient.php");
    exit; // Don't forget to exit after sending a header
}
...
if (isset($_POST['login'])) {
    ...
    if ($row && $row['password'] == $password) {
        ...
        $log->info("User {$icPatient} logged in successfully.");
        header("Location: patient/patient.php");
        exit;
    } else {
        $log->warning("Failed login attempt for user {$icPatient}.");
...
if (isset($_POST['signup'])) {
    ...
    if ($result) {
        $log->info("New user {$patientEmail} registered successfully.");
        echo "<script>alert('Register success. Please Login to make an appointment.');</script>";
    } else {
        $log->warning("Failed registration attempt for user {$patientEmail}.");
